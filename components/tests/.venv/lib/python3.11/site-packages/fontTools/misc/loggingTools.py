...
class LevelFormatter(logging.Formatter):
    ...
    format strings, allowing to customise the logger records appearance for
    specific levels.
    ...
    >>> logger.debug('this uses a custom format string')
    test [DEBUG] this uses a custom format string
    >>> logger.info('this also uses a custom format string')
    this also uses a custom format string
    >>> logger.warning("this one uses the default format string")
...
class Timer(object):
    ...
    If initialised with a logger, it can logger the elapsed time automatically
    upon exiting the with-statement.

    >>> import logging
    >>> logger = logging.getLogger("my-fancy-timer-logger")
    >>> configLogger(logger=logger, level="DEBUG", format="%(message)s", stream=sys.stdout)
    ...
    It can also be used as a function decorator to logger the time elapsed to run
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        """End the current lap. If timer has a logger, logger the time elapsed,
        ...
        # Allow logger handlers to see the individual parts to facilitate things
...
class ChannelsFilter(logging.Filter):
    """Provides a hierarchical filter for logger entries based on channel names.
...
class LogMixin(object):
    ...
    All instances of that class will have a ``logger`` property that returns
    ...
    ...     pass
    >>> a = MyClass()
    >>> isinstance(a.logger, logging.Logger)
    True
    >>> print(a.logger.name)
    ...
    ...     pass
    >>> b = AnotherClass()
    >>> isinstance(b.logger, logging.Logger)
    True
    >>> print(b.logger.name)
