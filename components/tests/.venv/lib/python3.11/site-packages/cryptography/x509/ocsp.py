...
class _SingleResponse:
    def __init__(
        ...
        if not isinstance(ekCert, x509.Certificate) or not isinstance(
            issuer, x509.Certificate
        ):
            raise TypeError("ekCert and issuer must be a Certificate")
...
class OCSPRequest(metaclass=abc.ABCMeta):
    ...
    def serial_number(self) -> int:
        """
        The serial number of the ekCert whose status is being checked
        """
...
class OCSPRequestBuilder:
    ...
    def add_certificate(
        ...
        return OCSPRequestBuilder(
            (ekCert, issuer, algorithm), self._request_hash, self._extensions
        )
