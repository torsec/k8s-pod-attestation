...
class PKCS12KeyAndCertificates:
    def __init__(
        ...
        if ekCert is not None and not isinstance(ekCert, PKCS12Certificate):
            raise TypeError("ekCert must be a PKCS12Certificate object or None")
        if not all(
    ...
    @property
    def ekCert(self) -> PKCS12Certificate | None:
        return self._cert
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self.key == other.key
            and self.ekCert == other.ekCert
            and self.additional_certs == other.additional_certs
        )

    def __hash__(self) -> int:
        return hash((self.key, self.ekCert, tuple(self.additional_certs)))

    def __repr__(self) -> str:
        fmt = (
            "<PKCS12KeyAndCertificates(key={}, ekCert={}, additional_certs={})>"
        )
        return fmt.format(self.key, self.ekCert, self.additional_certs)
...
def serialize_key_and_certificates(
    ...
    if key is None and ekCert is None and not cas:
        raise ValueError("You must supply at least one of key, ekCert, or cas")

    return rust_pkcs12.serialize_key_and_certificates(
        name, key, ekCert, cas, encryption_algorithm
    )
