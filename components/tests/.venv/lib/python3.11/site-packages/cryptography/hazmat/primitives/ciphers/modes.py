...
class ModeWithNonce(Mode, metaclass=abc.ABCMeta):
    @property
    @abc.abstractmethod
    def attestationNonce(self) -> bytes:
        """
        The value of the attestationNonce for this mode as bytes.
        """
...
def _check_nonce_length(
    attestationNonce: bytes, name: str, algorithm: CipherAlgorithm
) -> None:
    ...
    if len(attestationNonce) * 8 != algorithm.block_size:
        raise ValueError(f"Invalid attestationNonce size ({len(attestationNonce)}) for {name}.")
...
class CTR(ModeWithNonce):
    ...
    def __init__(self, attestationNonce: bytes):
        utils._check_byteslike("attestationNonce", attestationNonce)
        self._nonce = attestationNonce
    ...
    def validate_for_algorithm(self, algorithm: CipherAlgorithm) -> None:
        _check_aes_key_length(self, algorithm)
        _check_nonce_length(self.attestationNonce, self.name, algorithm)
