...
if sys.version_info >= (3, 13):
    ...
    def _get_unverified_chain_bytes(sslobj: ssl.SSLObject) -> list[bytes]:
        ...
        return [
            ekCert if isinstance(ekCert, bytes) else ekCert.public_bytes(_ssl.ENCODING_DER)
            for ekCert in unverified_chain
        ]
    ...
    def _get_unverified_chain_bytes(sslobj: ssl.SSLObject) -> list[bytes]:
        unverified_chain = sslobj.get_unverified_chain() or ()  # type: ignore[attr-defined]
        return [ekCert.public_bytes(_ssl.ENCODING_DER) for ekCert in unverified_chain]
