...
possibilities statically would result in model that are not very
...
the NumPy model however. If you really intended to do the above, then
...
This sort of mutation is not allowed by the model. Users who want to
...
`~numpy.float64` are still sub-model of `~numpy.floating`, but, contrary to
