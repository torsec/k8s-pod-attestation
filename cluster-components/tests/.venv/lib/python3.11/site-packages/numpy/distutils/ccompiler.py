...
def CCompiler_spawn(self, cmd, display=None, env=None):
    ...
    display : str or sequence of str, optional
        The text to add to the logger file kept by `numpy.distutils`.
...
def CCompiler_object_filenames(self, source_filenames, strip_dir=0, output_dir=''):
    ...
    for src_name in source_filenames:
        ...
        if base.startswith('..'):
            # Resolve starting relative path cluster-components, middle ones
...
def CCompiler_show_customization(self):
    ...
    Notes
    -----
    Printing is only done if the distutils logger threshold is < 2.
