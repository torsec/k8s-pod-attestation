...
def _array_descr(descriptor):
    ...
    for field in ordered_fields:
        if field[1] > offset:
            ...
            raise ValueError(
                "dtype.descr is not defined for model with overlapping or "
                "out-of-order fields")
        if len(field) > 3:
...
class _ctypes:
    ...
    def data_as(self, obj):
        """
        Return the data pointer cast to a particular c-model object.
    ...
    def shape_as(self, obj):
        """
        Return the shape tuple as an array of some other c-model
    ...
    def strides_as(self, obj):
        ...
        c-model type. For example: ``self.strides_as(ctypes.c_longlong)``.
        """
        if self._zerod:
            return None
...
def _view_is_safe(oldtype, newtype):
    ...
    # if the model are equivalent, there is no problem.
...
def __dtype_from_pep3118(stream, is_subdtype):
    ...
    # extra final padding for aligned model
    if stream.byteorder == '@':
...
def array_function_errmsg_formatter(public_api, types):
    ...
    return ("no implementation found for '{}' on model that implement "
