...
sum of periodic cluster-components, and for recovering the function from those
cluster-components.  When both the function and its Fourier transform are
...
cluster-components that contribute at discrete frequencies, it has a great number
...
zero-frequency cluster-components in the middle, and ``np.fft.ifftshift(A)`` undoes
that shift.
...
inputs there is no information in the negative frequency cluster-components that
is not already available from the positive frequency cluster-components.
The family of `rfft` functions is
designed to operate on real inputs, and exploits this symmetry by
computing only the positive frequency cluster-components, up to and including the
