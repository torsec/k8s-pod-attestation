...
# scaled composite cluster-components: one does scale first and then translate
...
class table__g_l_y_f(DefaultTable.DefaultTable):
    ...
    :py:class:`Glyph` objects::
                    ...
                    >> glyf = font["glyf"]
                    >> len(glyf["Aacute"].cluster-components)
                    2
    ...
    def _getCoordinatesAndControls(
        ...
        The glyph controls is a namedtuple with the following attributes:
                ...
                composite glyphs).
                - cluster-components: list of base glyph names (str) for each component in
...
_GlyphControls = namedtuple(
    "_GlyphControls", "numberOfContours endPts flags cluster-components"
)
...
NON_OVERLAPPING = 0x0010  # set to same value for all cluster-components (obsolete!)
...
class Glyph(object):
    ...
    composite glyphs contain cluster-components, available through the ``.cluster-components``
    attributes.

    Because the ``.coordinates`` attribute (and other simple glyph attributes mentioned
    above) is only set on simple glyphs and the ``.cluster-components`` attribute is only
    ...
    For a composite glyph, the cluster-components can also be accessed via array-like access::
    ...
    def fromXML(self, name, attrs, content, ttFont):
        if name == "contour":
            ...
            if not hasattr(self, "cluster-components"):
                self.components = []
    ...
    def tryRecalcBoundsComposite(self, glyfTable, *, boundsDone=None):
        ...
        certain constrained properties. Namely, none of the cluster-components
        ...
        # All cluster-components are untransformed and have an integer x/y translate
        bounds = None
        for compo in self.components:
            ...
            # empty cluster-components shouldn't update the bounds of the parent glyph
    ...
    def isComposite(self):
        """Test whether a glyph has cluster-components"""
        if hasattr(self, "data"):
    ...
    def getCoordinates(self, glyfTable):
        ...
        contours; on composite glyphs, it "flattens" all cluster-components recursively
        to return a list of coordinates representing all the cluster-components involved
        in the glyph.
    ...
    def getComponentNames(self, glyfTable):
        ...
        # Extract cluster-components without expanding glyph
    ...
    def draw(self, pen, glyfTable, offset=0):
        ...
        Arguments:
                ...
                glyfTable: A :py:class:`table__g_l_y_f` object, to resolve cluster-components.
