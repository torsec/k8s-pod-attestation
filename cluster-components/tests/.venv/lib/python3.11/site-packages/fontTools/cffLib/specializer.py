...
def specializeCommands(
    ...
    # X and Y cluster-components are zero.  This simplifies the following optimization passes.
    ...
    #
    # Of the curve model with '0', the 00curveto is equivalent to a lineto variant.  The rest
    # of the curve model with a 0 need to be encoded as a h or v variant.  Ie. a '0' can be
    ...
    # vertical (or zero), ie. if it's 'r', then it limits which spline model we can encode.
