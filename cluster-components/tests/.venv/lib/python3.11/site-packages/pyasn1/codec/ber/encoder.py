...
class SequenceEncoder(AbstractItemEncoder):
    ...
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        ...
        if LOG:
            LOG('%sencoding empty OPTIONAL cluster-components' % (
...
class ChoiceEncoder(AbstractItemEncoder):
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        if asn1Spec is None:
            ...
            if len(names) != 1:
                raise error.PyAsn1Error('%s cluster-components for Choice at %r' % (len(names) and 'Multiple ' or 'None ', value))
...
# Put in ambiguous & non-ambiguous model for faster codec lookup
TYPE_MAP = {
...
#: walks all its cluster-components recursively and produces a BER octet stream.
#:
#: Parameters
#: ----------
