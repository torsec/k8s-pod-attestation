...
func (s *Server) storeTPMCACertificate(c *gin.Context) {
	...
	decodedCert, err := base64.StdEncoding.DecodeString(req.TPMCaCertificate)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"message": "TPM CA Certificate is not valid base64", "status": model.Error})
		return
	}

	tpmCaCert, err := cryptoUtils.LoadCertificateFromPEM(decodedCert)
	if err != nil {
	...
	newTpmCaCert := &model.TPMCACertificate{
		...
		PEMCertificate: string(decodedCert),
	}
...
func (s *Server) insertTPMVendor(tpmManufacturer *model.TPMVendor) error {
	query := "INSERT INTO tpm_vendors (name, TCGIdentifier) VALUES (?, ?);"
	_, err := s.db.Exec(query, tpmManufacturer.Name, tpmManufacturer.TCGIdentifier)
	return err
}
