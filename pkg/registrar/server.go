...
type TPMCACertificate struct {
	...
	PEMCertificate []byte `json:"PEMCertificate"`
}
...
const (
	...
	StoreTPMCaCertificateUrl = "/tpm/ca/certificate/store"
	StoreTPMVendorUrl        = "/tpm/vendor/store"
)
...
func (s *Server) storeTPMVendor(c *gin.Context) {
	...
	c.JSON(http.StatusCreated, gin.H{"message": "New TPM vendor", "status": model.Success})
}

func (s *Server) storeTPMCACertificate(c *gin.Context) {
	var req model.NewTPMCaCertRequest
	if err := c.BindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"message": "Invalid request payload", "status": model.Error})
		return
	}

	tpmCaCert, err := cryptoUtils.LoadCertificateFromPEM(req.TPMCaCert)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"message": "TPM CA Certificate is not valid PEM", "status": model.Error})
		return
	}

	if tpmCaCert.Issuer.CommonName != tpmCaCert.Subject.CommonName {
		rootCaCert, err := s.getCertificateByCommonName(tpmCaCert.Issuer.CommonName)
		if err != nil {
			c.JSON(http.StatusUnauthorized, gin.H{"message": "TPM CA Certificate is intermediate and no Root CA exists for it", "status": model.Error})
			return
		}

		rootCaCertDecoded, err := cryptoUtils.LoadCertificateFromPEM(rootCaCert.PEMCertificate)
		if err != nil {
	...
	newTpmCaCert := &TPMCACertificate{
		CommonName:     tpmCaCert.Subject.CommonName,
		PEMCertificate: req.TPMCaCert,
	}

	err = s.insertCertificate(newTpmCaCert)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"message": "Failed to store TPM CA Certificate", "status": model.Error})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "New TPM CA Certificate successfully stored", "status": model.Error})
	return
...
func (s *Server) getCertificateByCommonName(commonName string) (*TPMCACertificate, error) {
...
// Insert a new TPM manufacturer
func (s *Server) insertTPMVendor(tpmManufacturer *cryptoUtils.TPMVendor) error {
	query := "INSERT INTO tpm_vendors (name, TCGIdentifier) VALUES (?, ?);"
	_, err := s.db.Exec(query, tpmManufacturer.Name, tpmManufacturer.TCGId)
	return err
}

// Insert a new certificate into the database
func (s *Server) insertCertificate(tpmCertificate *TPMCACertificate) error {
...
func (s *Server) getTenantByName(name string) (*Tenant, error) {
	var tenant Tenant
...
func (s *Server) insertTenant(tenant *Tenant) error {
...
func (s *Server) createTenant(c *gin.Context) {
	...
	pubKeyExists, err := s.tenantExistsByPublicKey(string(newTenantRequest.PublicKey))
	if err != nil {
...
func (s *Server) verifyTenantSignature(c *gin.Context) {
	...
	err = verifySignatureRequest.Signature.Verify(tenantPublicKey, verifySignatureRequest.Message)
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"message": "Failed to verify signature", "status": model.Error})
		return
	}
...
func (s *Server) createWorker(c *gin.Context) {
	...
	AIKExists, err := s.workerExistsByAIK(string(newWorker.AIK))
	if err != nil {
...
func (s *Server) verifyWorkerSignature(c *gin.Context) {
	...
	err = verifySignatureRequest.Signature.Verify(workerPublicKey, verifySignatureRequest.Message)
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"message": "Failed to verify signature", "status": model.Error})
		return
	}
...
func (s *Server) initTPMVendors() error {
	...
	for _, vendor := range getKnownTPMManufacturers() {
		_, err := query.Exec(vendor.Name, vendor.TCGId)
		if err != nil {
...
func (s *Server) Start() {
	...
	// Define routes for Certificate handling
	s.router.POST(StoreTPMCaCertificateUrl, s.storeTPMCACertificate)
	s.router.POST(StoreTPMVendorUrl, s.storeTPMVendor)
