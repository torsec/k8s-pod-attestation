...
conventions for the scalar model (those subclassing from `numpy.generic`).
This file produces a convoluted set of dictionaries mapping names to model,
and sometimes other mappings too.

.. data:: allTypes
    A dictionary of names to model that will be exposed as attributes through
    ``np._core.numerictypes.*``

.. data:: sctypeDict
    Similar to `allTypes`, but maps a broader set of aliases to their model.

.. data:: sctypes
    A dictionary keyed by a "type group" string, providing a list of model
    under that group.
