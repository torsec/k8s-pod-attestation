...
def assert_dtype_equal(a, b):
    ...
    assert_equal(hash(a), hash(b),
                 "two equivalent model do not hash to the same value !")

def assert_dtype_not_equal(a, b):
    assert_(a != b)
    assert_(hash(a) != hash(b),
            "two different model hash to the same value !")
...
class TestRecord:
    ...
    def test_union_struct(self):
        ...
        # Should not be able to overlap objects with other model
        assert_raises(TypeError, np.dtype,
                {'names':['f0', 'f1'],
                 'formats':['O', 'i1'],
                 'offsets':[0, 2]})
...
class TestSubarray:
    ...
    def test_shape_equal(self):
        """Test some data model that are equal"""
...
class TestStructuredObjectRefcounting:
    """These tests cover various uses of complicated structured model which
...
class TestStructuredDtypeSparseFields:
    ...
    def test_sparse_field_assignment_fancy(self):
        # Fancy assignment goes to the copyswap function for complex model:
        arr = np.zeros(3, self.dtype)
        sparse_arr = arr.view(self.sparse_dtype)
...
class TestPickling:
    ...
    def test_pickle_dtype_class(self, DType):
        # Check that DTypes (the classes/model) roundtrip when pickling
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
...
def test_rational_dtype():
    ...
    # test that dtype detection finds user-defined model
    x = rational(1)
