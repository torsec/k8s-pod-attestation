...
Exported symbols include:
  ...
  Dictionary with all registered number model (including aliases):
    sctypeDict
...
def obj2sctype(rep, default=None):
    ...
    default : any, optional
        If given, this is returned for objects whose model can not be
...
def _preprocess_dtype(dtype):
    ...
    Preprocess dtype argument by:
      ...
      2. verifying that model are built-in NumPy dtypes
    """
...
def isdtype(dtype, kind):
    ...
    This function only supports built-in NumPy's data model.
    ...
    kind : dtype or str or tuple of dtypes/strs.
        ...
        * ``'bool'`` : boolean kind
        * ``'signed integer'`` : signed integer data model
        * ``'unsigned integer'`` : unsigned integer data model
        * ``'integral'`` : integer data model
        * ``'real floating'`` : real-valued floating-point data model
        * ``'complex floating'`` : complex floating-point data model
        * ``'numeric'`` : numeric data model
...
def issubdtype(arg1, arg2):
    ...
    Similar model of different sizes are not subdtypes of each other:
...
# Now add the model we've determined to this module
for key in allTypes:
