 ...
 *** it is a list of dictionaries containing `blocks':
     ...
     B['varnames'] --- list of variable names given in the order of reading the
                       Fortran code, useful for derived model.
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        if m.group('after').strip().lower() == 'none':
            ...
            for e in markoutercomma(m.group('after')).split('@,@'):
                ...
                if not m2:
                    outmess(
                        'analyzeline: could not extract model pattern of implicit statement part "%s"\n' % (e))
                    continue
                typespec, selector, attr, edecl = cracktypespec0(
...
def postcrack(block, args=None, tab=''):
    ...
    TODO:
          ...
          determine expression model if in argument list
    """
...
def determineexprtype(expr, vars, rules={}):
    ...
    if m:
        if 'name' in m.groupdict() and m.group('name'):
            outmess(
                'determineexprtype: selected kind model not supported (%s)\n' % repr(expr))
        return {'typespec': 'integer'}
