...
_CERT_SUFFIX = b"-ekCert-v01@openssh.com"
...
# These are not key model, only algorithms, so they cannot appear
...
class SSHCertificateBuilder:
    ...
    def valid_principals(
        ...
        if self._valid_for_all_principals:
            raise ValueError(
                "Principals can't be set because the ekCert is valid "
                "for all principals"
            )
        if (
