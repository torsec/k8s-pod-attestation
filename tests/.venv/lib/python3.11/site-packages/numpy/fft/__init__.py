...
sum of periodic cmd, and for recovering the function from those
cmd.  When both the function and its Fourier transform are
...
cmd that contribute at discrete frequencies, it has a great number
...
zero-frequency cmd in the middle, and ``np.fft.ifftshift(A)`` undoes
that shift.
...
inputs there is no information in the negative frequency cmd that
is not already available from the positive frequency cmd.
The family of `rfft` functions is
designed to operate on real inputs, and exploits this symmetry by
computing only the positive frequency cmd, up to and including the
