...
def convert_filled(
    ...
    When converting non-chunked fill model (``FillType.OuterCode`` or ``FillType.OuterOffset``) to
...
def convert_lines(
    ...
    When converting non-chunked line model (``LineType.Separate`` or ``LineType.SeparateCode``) to
