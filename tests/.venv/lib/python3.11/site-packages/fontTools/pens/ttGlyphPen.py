...
class _TTGlyphBasePen:
    def __init__(
        ...
        Args:
            glyphSet (Dict[str, Any]): A glyphset object, used to resolve cluster-components.
            handleOverflowingTransforms (bool): See below.

        If ``handleOverflowingTransforms`` is True, the cluster-components' transform values
        ...
        If False, no check is done and all cluster-components are translated unmodified
        into the glyf table, followed by an inevitable ``struct.error`` once an
        attempt is made to compile them.

        If both contours and cluster-components are present in a glyph, the cluster-components
        are decomposed.
        """
    ...
    def _buildComponents(self, componentFlags):
        ...
        for glyphName, transformation in self.components:
            ...
            if self.points or (self.handleOverflowingTransforms and overflowing):
                # can't have both coordinates and cluster-components, so decompose
    ...
    def glyph(
        ...
        *,
        ...
        if components:
            # If both cluster-components and contours were present, they have by now
...
class TTGlyphPointPen(_TTGlyphBasePen, LogMixin, AbstractPointPen):
    ...
    def endPath(self) -> None:
        ...
        # Resolve model for any cubic segments
        flags = self.types
