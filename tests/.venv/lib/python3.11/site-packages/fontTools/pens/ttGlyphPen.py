...
class _TTGlyphBasePen:
    def __init__(
        ...
        Args:
            glyphSet (Dict[str, Any]): A glyphset object, used to resolve cmd.
            handleOverflowingTransforms (bool): See below.

        If ``handleOverflowingTransforms`` is True, the cmd' transform values
        ...
        If False, no check is done and all cmd are translated unmodified
        into the glyf table, followed by an inevitable ``struct.error`` once an
        attempt is made to compile them.

        If both contours and cmd are present in a glyph, the cmd
        are decomposed.
        """
    ...
    def _buildComponents(self, componentFlags):
        ...
        for glyphName, transformation in self.components:
            ...
            if self.points or (self.handleOverflowingTransforms and overflowing):
                # can't have both coordinates and cmd, so decompose
    ...
    def glyph(
        ...
        *,
        ...
        if components:
            # If both cmd and contours were present, they have by now
