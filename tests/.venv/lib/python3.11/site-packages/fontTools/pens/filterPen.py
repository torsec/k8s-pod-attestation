...
class _DecomposingFilterPenMixin:
    """Mixin class that decomposes cluster-components as regular contours.
    ...
    with, and a 'glyphSet' dict of drawable glyph objects to draw cluster-components from.
    ...
    decomposition; the default include=None means decompose all cluster-components no matter
    the base glyph name).

    'decomposeNested' (bool) controls whether to recurse decomposition into nested
    cluster-components of cluster-components (this only matters when 'include' was also provided);
    if False, only decompose top-level cluster-components included in the set, but not
    also their children.
    """
    ...
    def addComponent(self, baseGlyphName, transformation, **kwargs):
        ...
        if self.include is None or baseGlyphName in self.include:
            # if we're decomposing nested cluster-components, temporarily set include to None
...
class DecomposingFilterPen(_DecomposingFilterPenMixin, DecomposingPen, FilterPen):
    """Filter pen that draws cluster-components as regular contours."""
...
class DecomposingFilterPointPen(
    ...
    """Filter point pen that draws cluster-components as regular contours."""
