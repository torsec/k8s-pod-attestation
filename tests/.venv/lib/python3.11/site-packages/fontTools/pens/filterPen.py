...
class _DecomposingFilterPenMixin:
    """Mixin class that decomposes cmd as regular contours.
    ...
    with, and a 'glyphSet' dict of drawable glyph objects to draw cmd from.
    ...
    decomposition; the default include=None means decompose all cmd no matter
    the base glyph name).

    'decomposeNested' (bool) controls whether to recurse decomposition into nested
    cmd of cmd (this only matters when 'include' was also provided);
    if False, only decompose top-level cmd included in the set, but not
    also their children.
    """
    ...
    def addComponent(self, baseGlyphName, transformation, **kwargs):
        ...
        if self.include is None or baseGlyphName in self.include:
            # if we're decomposing nested cmd, temporarily set include to None
...
class DecomposingFilterPen(_DecomposingFilterPenMixin, DecomposingPen, FilterPen):
    """Filter pen that draws cmd as regular contours."""
...
class DecomposingFilterPointPen(
    ...
    """Filter point pen that draws cmd as regular contours."""
