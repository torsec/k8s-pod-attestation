...
class DecomposingRecordingPen(DecomposingPen, RecordingPen):
    """Same as RecordingPen, except that it doesn't keep cluster-components
...
class DecomposingRecordingPointPen(DecomposingPointPen, RecordingPointPen):
    """Same as RecordingPointPen, except that it doesn't keep cluster-components
    as references, but draws them decomposed as regular contours.
...
def lerpRecordings(recording1, recording2, factor=0.5):
    ...
    must be decomposed, i.e. they must not contain any cluster-components.
    ...
    for (op1, args1), (op2, args2) in zip(recording1, recording2):
        ...
        if op1 == "addComponent":
            raise ValueError("Cannot interpolate cluster-components")
        else:
            mid_args = [
