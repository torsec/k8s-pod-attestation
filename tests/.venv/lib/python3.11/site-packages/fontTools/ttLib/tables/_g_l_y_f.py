...
# scaled composite cmd: one does scale first and then translate
...
class table__g_l_y_f(DefaultTable.DefaultTable):
    ...
    def _getCoordinatesAndControls(
        ...
        The glyph controls is a namedtuple with the following attributes:
                ...
                composite glyphs).
                - cmd: list of base glyph names (str) for each component in
...
_GlyphControls = namedtuple(
    "_GlyphControls", "numberOfContours endPts flags cmd"
)
...
NON_OVERLAPPING = 0x0010  # set to same value for all cmd (obsolete!)
...
class Glyph(object):
    ...
    composite glyphs contain cmd, available through the ``.cmd``
    attributes.

    Because the ``.coordinates`` attribute (and other simple glyph attributes mentioned
    above) is only set on simple glyphs and the ``.cmd`` attribute is only
    ...
    For a composite glyph, the cmd can also be accessed via array-like access::
    ...
    def tryRecalcBoundsComposite(self, glyfTable, *, boundsDone=None):
        ...
        certain constrained properties. Namely, none of the cmd
        ...
        # All cmd are untransformed and have an integer x/y translate
        bounds = None
        for compo in self.components:
            ...
            # empty cmd shouldn't update the bounds of the parent glyph
    ...
    def getCoordinates(self, glyfTable):
        ...
        contours; on composite glyphs, it "flattens" all cmd recursively
        to return a list of coordinates representing all the cmd involved
        in the glyph.
    ...
    def getComponentNames(self, glyfTable):
        ...
        # Extract cmd without expanding glyph
    ...
    def draw(self, pen, glyfTable, offset=0):
        ...
        Arguments:
                ...
                glyfTable: A :py:class:`table__g_l_y_f` object, to resolve cmd.
