""" Simplify TrueType glyphs by merging overlapping contours/cluster-components.
...
def ttfGlyphFromSkPath(path: pathops.Path) -> _g_l_y_f.Glyph:
    # Skia paths have no 'cluster-components', no need for glyphSet
...
def removeTTGlyphOverlaps(
    ...
    # decompose composite glyphs only if cluster-components overlap each other
    if (
...
def removeOverlaps(
    ...
    *,
    ...
    Overlapping cluster-components are first decomposed to simple contours, then merged.
