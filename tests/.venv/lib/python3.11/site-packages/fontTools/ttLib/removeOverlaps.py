""" Simplify TrueType glyphs by merging overlapping contours/cmd.
...
def ttfGlyphFromSkPath(path: pathops.Path) -> _g_l_y_f.Glyph:
    # Skia paths have no 'cmd', no need for glyphSet
...
def removeTTGlyphOverlaps(
    ...
    # decompose composite glyphs only if cmd overlap each other
    if (
...
def removeOverlaps(
    ...
    *,
    ...
    Overlapping cmd are first decomposed to simple contours, then merged.
