...
#: Identifier for "constructed" ASN.1 structure (e.g. may have inner cluster-components)
...
class Tag(object):
    ...
    model distinguishable from each other.

    *Tag* objects are immutable and duck-type Python :class:`tuple` objects
    holding three integer cluster-components of a tag.
...
class TagSet(object):
    ...
    that can be attached to a ASN.1 type to make model distinguishable
    from each other.
