...
def get_trust_status(pod_uid: str = Query(..., description="UID of the pod")):
    # Step 1: Find pod name and node name by UID
    try:
    ...
    pod = next((p for p in pods["items"] if p["metadata"].get("uid") == pod_uid), None)
    if not pod:
